...
# 🧩 Assignment 1 — Promise Based API Orchestrator
...
function fetchComments() {
  return new Promise((_, reject) => {
    setTimeout(() => reject("Comments API failed ❌"), 1200);
  });
}
```
...
# 🧩 Assignment 2 — Async/Await Dashboard Loader
...
### Task 1 — Sequential Dashboard Load
...
❌ No `.then()` chaining
❌ No global variables (except given one)
✅ Must use `try/catch`
✅ Must use `Promise.all`
✅ Must write reusable `retry` utility
